...
function getNumbers(str) {
    ...
    const numbers = chars.filter(char => isNaN(parseInt(char)) ? false : true);
    return numbers.map(num => parseInt(num));
}


function convertDigits(digit1, digit2) {
    return parseInt(`${digit1}${digit2}`)
}

const sum = input
    .map(str => getNumbers(str))
    .map(numArray => convertDigits(numArray[0], numArray[numArray.length - 1]))
...
// parse strings for digits (sixteen is a decoy? that's not a digit)
// convert string numbers into digits

const numsAsWords = ['superinvalidlsdfkjlsdkjf', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'];

function firstAndLasts(string) {
    const containedStrings = numsAsWords.filter(word => string.includes(word));
    if (containedStrings.length === 0) {
        const first = getNumbers(string).shift();
        const second = getNumbers(string).pop();
        return convertDigits(first, second);
    }
    const indices = containedStrings.map(word => {
        return {
            word,
            index: string.indexOf(word),
            lastIndex: string.lastIndexOf(word)
        }
    });
    const firstTextNumber = indices.sort((a, b) => a.index - b.index)[0];
    const lastTextNumber = indices.sort((a, b) => b.lastIndex - a.lastIndex)[0];
    const stringWithFirst = string.replaceAll(firstTextNumber.word, numsAsWords.indexOf(firstTextNumber.word));
    const firstDigit = getNumbers(stringWithFirst).shift();
    const stringWithLast = string.replaceAll(lastTextNumber.word, numsAsWords.indexOf(lastTextNumber.word));
    const secondDigit = getNumbers(stringWithLast).pop();

    return convertDigits(firstDigit, secondDigit);
}


const answer = input.map(str => firstAndLasts(str)).reduce((acc, curr) => acc + curr, 0);
